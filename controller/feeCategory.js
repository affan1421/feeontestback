...
const createFeeCategory = async (req, res, next) => {
	try {
		...
		res
			.status(201)
			.json(SuccessResponse(feeCategory, 1, 'Created Successfully'));
	} catch (error) {
...
const updateFeeCategory = async (req, res, next) => {
	try {
		...
		if (!feeCategory) {
			return res
				.status(404)
				.json(new ErrorResponse('Fee Category Not Found', 404));
		}
		res
			.status(200)
			.json(SuccessResponse(feeCategory, 1, 'Updated Successfully'));
	} catch (error) {
		return next(new ErrorResponse('Something Went Wrong', 500));
	}
};
