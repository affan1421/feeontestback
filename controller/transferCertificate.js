...
const getTcReason = async (req, res, next) => {
	...
	try {
		...
		const result = tcReasonModal
			.find({ schoolId }, 'reason')
			.skip(skip)
			.limit(pageSize);
...
const addTcReason = async (req, res, next) => {
	...
	try {
		...
		if (existingReason)
			return next(new ErrorResponse('Reason already exists', 403));
		const result = await tcReasonModal.create({ reason, schoolId });
		res
			.status(200)
			.json(SuccessResponse(result, 1, 'Tc reason created successfully'));
	} catch (error) {
...
async function updateTcReason(req, res, next) {
	...
	try {
		...
		const existingReason = await tcReasonModal.findOne({ reason });
		if (existingReason)
			return next(new ErrorResponse('This reason name already exists', 403));
		...
		res
			.status(200)
			.json(SuccessResponse(result, 1, 'Tc reasons updated successfully'));
	} catch (error) {
		return next(new ErrorResponse('Something went wrong', 500));
	}
}
